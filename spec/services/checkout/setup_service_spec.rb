...
RSpec.describe Checkout::SetupService do
  ...
  let(:credit_card) { { name: "#{FFaker::Name.first_name} #{FFaker::Name.last_name}", card_number: Faker::Number.number(16),
                        cvv: Faker::Number.number(3), expiration_month_year: '01/23' } }

  let(:addresses_params) { { billing: { firstname: FFaker::Name.first_name, lastname: FFaker::Name.last_name,
  ...
  #   setup_service = Checkout::SetupService.new(current_user: user, current_order: order, step: :addresses, params: addresses_params)
  #   setup_service.call
